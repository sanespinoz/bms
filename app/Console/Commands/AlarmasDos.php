...
class AlarmasUno extends Command
{
     ...
     * @var string
     */
    protected $signature = 'alarmas:uno';
    /**
     * The console command description.
     *
     * @var string
     */
    protected $description = 'Este comando lee las alarmas en los grupos del piso 1';
    /**
...
//Sector A1
        for ($i = 1; $i < 4; $i++) 
        {
            $alarmas = DB::connection('netx')
                ...
                ->where('ITEMID', 'like', '%BMS\Alarmas\piso 1\A1\G'.$i)
            ...
            if($alarmas != []) 
            {
                ...
                foreach ($luminarias as $v) 
                {
                    ...
                    \Log::info('insertando ALARMAS del sector A1 Piso 1' . \Carbon\Carbon::now());
                }
            }
            //echo ($i);
        }
//Sector A2
        for ($i = 1; $i < 4; $i++) 
        {
                ...
                ->select(DB::raw('(NUM_VALUE) as m'))
                ->join('dbo.NETX_HISTORICAL_VALUE', 'NETX_DEFINITION.handle', '=', 'NETX_HISTORICAL_VALUE.handle')
                ->where('ITEMID', 'like', '%BMS\Alarmas\piso 1\A2\G'.$i)
                ->where('NETX_HISTORICAL_VALUE.NUM_VALUE','1')
                ->where('LOCAL_DATE' ,'>', DB::raw('DATEADD([minute], -10, GETDATE())'))
                ->where('LOCAL_DATE' ,'<',  DB::raw('GETDATE()'))
                ->orderBy(DB::raw('LOCAL_DATE'), 'desc')
            ...
            if ($alarmas != []) 
            {
                ...
                foreach ($luminarias as $v) 
                {
                    ...
                    \Log::info('insertando alarmas del sector A2 Piso 1' . \Carbon\Carbon::now());
                }
            }
        }
            //Sector A3N
            ...
            $alarmas = DB::connection('netx')
                ...
                ->select(DB::raw('(NUM_VALUE) as m'))
                ->join('dbo.NETX_HISTORICAL_VALUE', 'NETX_DEFINITION.handle', '=', 'NETX_HISTORICAL_VALUE.handle')
                ->where('ITEMID', 'like', '%BMS\Alarmas\piso 1\A3N\G'.$i)
                ->where('NETX_HISTORICAL_VALUE.NUM_VALUE','1')
                ->where('LOCAL_DATE' ,'>', DB::raw('DATEADD([minute], -10, GETDATE())'))
                ->where('LOCAL_DATE' ,'<',  DB::raw('GETDATE()'))
                ->orderBy(DB::raw('LOCAL_DATE'), 'desc')
                ...
                foreach ($grupo as $g) {
                    ...
                    foreach ($luminarias as $v) 
                    {
                        ...
                        \Log::info('insertando alarmas del sector A3N Piso 1' . \Carbon\Carbon::now());
                    }
                }
            }
                //Sector A3S
                ...
                ->select(DB::raw('(NUM_VALUE) as m'))
                ->join('dbo.NETX_HISTORICAL_VALUE', 'NETX_DEFINITION.handle', '=', 'NETX_HISTORICAL_VALUE.handle')
                ->where('ITEMID', 'like', '%BMS\Alarmas\piso 1\A3S\G'.$i)
                ->where('NETX_HISTORICAL_VALUE.NUM_VALUE','1')
                ->where('LOCAL_DATE' ,'>', DB::raw('DATEADD([minute], -10, GETDATE())'))
                ->where('LOCAL_DATE' ,'<',  DB::raw('GETDATE()'))
                ->orderBy(DB::raw('LOCAL_DATE'), 'desc')
                ...
                foreach ($grupo as $g) {
                    $gid = $g->id;
                        ...
                        foreach ($luminarias as $v) 
                        {
                            ...
                            \Log::info('insertando alarmas del sector A3S Piso 1' . \Carbon\Carbon::now());
                        }
                    }
                }
            ...
            $alarmas = DB::connection('netx')
                ...
                ->select(DB::raw('(NUM_VALUE) as m'))
                ->join('dbo.NETX_HISTORICAL_VALUE', 'NETX_DEFINITION.handle', '=', 'NETX_HISTORICAL_VALUE.handle')
                ->where('ITEMID', 'like', '%BMS\Alarmas\piso 1\A4N\G'.$i)
                ->where('NETX_HISTORICAL_VALUE.NUM_VALUE','1')
                ->where('LOCAL_DATE' ,'>', DB::raw('DATEADD([minute], -10, GETDATE())'))
                ->where('LOCAL_DATE' ,'<',  DB::raw('GETDATE()'))
                ->orderBy(DB::raw('LOCAL_DATE'), 'desc')
                ...
                foreach ($grupo as $g) {
                    $gid = $g->id;
                            ...
                            foreach ($luminarias as $v) 
                            {
                                ...
                                \Log::info('insertando alarmas del sector A4N Piso 1' . \Carbon\Carbon::now());
                            }
                        }
            ...
            $alarmas = DB::connection('netx')
                ...
                ->select(DB::raw('(NUM_VALUE) as m'))
                ->join('dbo.NETX_HISTORICAL_VALUE', 'NETX_DEFINITION.handle', '=', 'NETX_HISTORICAL_VALUE.handle')
                ->where('ITEMID', 'like', '%BMS\Alarmas\piso 1\A4S\G'.$i)
                ->where('NETX_HISTORICAL_VALUE.NUM_VALUE','1')
                ->where('LOCAL_DATE' ,'>', DB::raw('DATEADD([minute], -10, GETDATE())'))
                ->where('LOCAL_DATE' ,'<',  DB::raw('GETDATE()'))
                ->orderBy(DB::raw('LOCAL_DATE'), 'desc')
                ...
                foreach ($grupo as $g) {
                    $gid = $g->id;
                                ...
                                foreach ($luminarias as $v) 
                                {
                                    ...
                                    \Log::info('insertando alarmas del sector A4S Piso 1' . \Carbon\Carbon::now());
                                }
                            }
