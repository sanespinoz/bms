...
class ContadorPersonasCero extends Command
{
     ...
     * @var string
     */
    protected $signature = 'contar:cero';
...
//Sector A3N
        $prompersonas = DB::connection('netx')
            ...
            ->where('ITEMID', 'like', '%NETx\VIRTUAL\BMS\Personas\Contador\Piso 0\A3N%')
...
//Actualizo
        ...
        $prompersonas = DB::connection('netx')
            ...
            ->select(DB::raw('AVG(NUM_VALUE) as pp'))
            ->join('dbo.NETX_HISTORICAL_VALUE', 'NETX_DEFINITION.handle', '=', 'NETX_HISTORICAL_VALUE.handle')
            ->where('ITEMID', 'like', '%NETx\VIRTUAL\BMS\Personas\Contador\Piso 0\A3S%')
            ->where(DB::raw('CONVERT(date, LOCAL_DATE)'), '=', DB::raw('CONVERT(date, GETDATE())'))
            ->get();
        foreach ($prompersonas as $e) {
            $prompersons = (int) $e->pp;
...
//Actualizo
        ...
        $prompersonas = DB::connection('netx')
            ...
            ->select(DB::raw('AVG(NUM_VALUE) as pp'))
            ->join('dbo.NETX_HISTORICAL_VALUE', 'NETX_DEFINITION.handle', '=', 'NETX_HISTORICAL_VALUE.handle')
            ->where('ITEMID', 'like', '%NETx\VIRTUAL\BMS\Personas\Contador\Piso 0\A4S%')
            ->where(DB::raw('CONVERT(date, LOCAL_DATE)'), '=', DB::raw('CONVERT(date, GETDATE())'))
            ->get();
        foreach ($prompersonas as $e) {
            $prompersons = (int) $e->pp;
...
//Actualizo
        ...
        $prompersonas = DB::connection('netx')
            ...
            ->select(DB::raw('AVG(NUM_VALUE) as pp'))
            ->join('dbo.NETX_HISTORICAL_VALUE', 'NETX_DEFINITION.handle', '=', 'NETX_HISTORICAL_VALUE.handle')
            ->where('ITEMID', 'like', '%NETx\VIRTUAL\BMS\Personas\Contador\Piso 0\A4N%')
            ->where(DB::raw('CONVERT(date, LOCAL_DATE)'), '=', DB::raw('CONVERT(date, GETDATE())'))
            ->get();
        foreach ($prompersonas as $e) {
            $prompersons = (int) $e->pp;
